<template>
  ...
  <div class="hello">
      ...
      <input type="button" value="トレンドライン削除" class="btn-gradient-radius"　@click="uploading()">  
             <input type="button" value='概要' class="btn-gradient-radius"　 @click="openModal()" >
...
export default {
  ...
  methods:{
    ...
    image_path: function(slug){
      try{
        console.log("@/assets/" + slug + ".png")

        return require("@/assets/" + slug + "T.png");
      }catch(e){
    ...
    //トレンドライン削除のボタンをクリックすると、トレンドラインに関係する描写が削除される。
    uploading:function() { 
        d3.selectAll("svg>g>g.ohlc>path.trendline_l").remove() //すべての描写したトレンドラインを削除する。
        d3.selectAll("svg>g>text").remove()  //トレンドラインに関連するテキストを削除する。
        console.log("remove")
    ...
    executeall: function(type,rang) {
      ...
      var valueText = ohlcSelection.append('text')
            .style("text-anchor", "end")
            .attr("class", "coords")
            .attr("x", 30 - 5)
            .attr("y", 15);
      ...
      //***** (4-2)ろうそく足の上にカーソルがoverしたときに日付とclose値を表示させる関数 ******//
        ...
        var lineput=function(){  
   
          var bands=[];
          console.log("lineput")
          //ろうそく足をクリックしたとき、その座標をbandsに配列として格納する。
          var pos=svg.on("click", function(){
            var posX=d3.mouse(this)[0]
            var posY=d3.mouse(this)[1]
            bands.push({ POSX: posX, POSY: posY }); 

            trendline_put(bands,posX,posY) //トレンドラインを描写するDOM(htmlコード)を作成する。
            //firstクリックのとき
            if(bands.length==1) {
              trendline_put(bands,posX,posY)
            }
            //secondクリックのとき
            if(bands.length==2) {
              trendline_put(plotconv(bands),posX,posY);　//plotconvを実施し、描写する直線を伸ばす。
              bands=[] ; //トレンドラインの座標情報を削除する。(次のトレンドラインを描写するため)
              console.log("bands erase");
            }
        ...
        //クリックして取得されたトレンドラインに関する座標情報について、トレンドラインを伸ばすために座標変換を実施する。
        var plotconv=function(bands){
          console.log("plotconv")
          var lx0=bands[0].POSX //1stクリックX座標
          var ly0=bands[0].POSY //1stクリックY座標
          var lx1=bands[1].POSX //2ndクリックX座標
          var ly1=bands[1].POSY //2ndクリックY座標
          var slp=(ly1-ly0)/(lx1-lx0)
          var lx2=0   //伸展直線開始X座標
          var ly2=-slp*lx1+ly1 //伸展直線傾き
          var lx3=x1 //伸展直線終了X座標
          var ly3=slp*x1-slp*lx1+ly1  //伸展直線終了Y座標
          console.log("RET")
          console.log([{ POSX: lx2, POSY: ly2 }, { POSX: lx3, POSY: ly3 }   ]) 
          return [{ POSX: lx2, POSY: ly2 }, { POSX: lx3, POSY: ly3 }   ]
        }



          //(4)トレンドラインを描写する。
          var treline = d3.line().x(function(d) { return d.POSX }) .y(function(d) { return d.POSY; });
          var trendline_put = function(bands,posX,posY){
            
            ohlcSelection.append("path")
                         .datum(bands)
                         .attr("class", "trendline_l")
                         .attr("d",treline)
                        svg.append("text")      
                        .attr("x",posX-100)
                        .attr("y",posY+10)
                        .text("OK") //クリックが完了し、座標が取得できればOKを出力する。

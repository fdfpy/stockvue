...
class Dict(DictTop):
    ...
    def mean_vola_dict(self,bx):  # 各銘柄の過去1年の株価上昇率と1日単位でのボラティリティーの関係をグラフ化する。
        ...
        bx.plot([etf_info['EXPECTATION'][0], etf_info['EXPECTATION'][0]], [0, 1.05 * np.max(y_num)], '--',color="blue")
        bx.plot([np.min(x_num) - 2, 1.05 * np.max(x_num)], [etf_info['SIGMA'][0], etf_info['SIGMA'][0]], '--',color="blue")
